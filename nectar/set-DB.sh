...
echo "== Set variables =="
declare -a nodes=(172.20.0.2 172.20.0.3)
...
# localhost = 127.0.0.1 in all computer
...
for (( i=0; i<${size}; i++ )); do
  if [ "${nodes[${i}]}" != "${master_node}" ]; then
    curl -X POST -H 'Content-Type: application/json' http://${user}:${pass}@:${master_port}/_cluster_setup \
...
# curl -XPUT "http://user:pass@127.0.0.1:5984/twitter"